...
const rows: { Icon: IconType; text: string }[] = [
  {
    Icon: FiInfo,
    text: "A dental implant is a small, biocompatible titanium post that is surgically placed into the jawbone to replace the root of a missing tooth.",
  },
  {
    Icon: FiDroplet,
    text: "Over a healing period, the implant gradually fuses with the surrounding bone in a process called osseointegration, creating a stable foundation. A custom-made crown is then attached on top to restore the look and function of a natural tooth.",
  },
  {
    Icon: FiShield,
    text: "Implants can be used to replace a single tooth, several teeth, or to support a fixed set of teeth. Unlike removable options, a dental implant is fixed in place and is designed to function much like a natural tooth, while also helping to preserve the surrounding jawbone.",
  },
];

export default function DentalImplantsWhyChooseUsSection() {
  return (
    <section
      className="relative overflow-hidden"
      style={{
        backgroundImage: "url('/dental-treatment.png')",
      ...
      >
        <div className="w-full lg:max-w-[54%] xl:max-w-[50%]">
          ...
          >
            What is a
            <br />
            <span className="text-[#354C9C]">Dental Implant?</span>
          </h2>
          ...
          <div className="mt-5 sm:mt-7 lg:mt-8 space-y-3 sm:space-y-5">
            {rows.map((row, i) => (
              <div key={i} className="flex items-start gap-3 sm:gap-4">
                ...
                <div
                  className="flex-1 bg-white rounded-xl sm:rounded-2xl
                                border border-[#D8DDE8]
                                shadow-[0_2px_16px_rgba(53,76,156,0.07)]
                                px-3 sm:px-5 py-2.5 sm:py-4"
                >
                  <p
                    className="text-[#555555] leading-[1.65]
